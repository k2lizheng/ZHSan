...
namespace GameManager
{
    ...
    public static class CacheManager
    {
        ...
        public static int GetTempTextureW(string name)
        {
            Texture2D pic = LoadTexture(name);
            return pic.Width;
        }
        public static int GetTempTextureH(string name)
        {
            Texture2D pic = LoadTexture(name);
            return pic.Height;
        }
        ...
        public static void DrawZhsanAvatar(Person person, string type, Rectangle pos, Color color, float depth)
        {
            ...
            //float pictureID = (float)((int)(person.PictureIndex*100))/100;
            float pictureID = (float)Math.Round(person.PictureIndex, 2);
            //if (person.Age >= 50)
            //{
            //    pictureID += 0.5f;
            //}
