...
#pragma warning restore CS0067 // The event 'Architecture.OnRewardPersons' is never used
        ...
        private void AITreasure()
        {
            if (GameObject.Chance(Session.Parameters.AITreasureChance) && !Session.Current.Scenario.IsPlayer(this.BelongedFaction))
            {
                ...
                if (((this.BelongedFaction.Leader != null) && (this.BelongedFaction.Leader.TreasureCount > Session.Parameters.AITreasureCountMax)) && this.HasTreasureToAward())
                {
                    ...
                    {
                        if ((person == this.BelongedFaction.Leader) || (person.TreasureCount > person.TotalTitleLevel * Session.Parameters.AITreasureCountCappedTitleLevelMultiply))
                        {
                            continue;
                        }
                        ...
                        {
                            foreach (Treasure treasure in this.BelongedFaction.Leader.Treasures.GetList())
                            {
                                if (treasure.Worth < Session.Parameters.AIGiveTreasureMaxWorth && !person.HasTreasureforGroup(treasure.TreasureGroup))
                                {
        ...
        */
        ...
        public void BeginToBuildAFacility(FacilityKind facilityKind)
        {
            ...
            this.BuildingDaysLeft = Math.Max(1, (int)(Math.Ceiling((double)facilityKind.Days / (double)Session.Parameters.DayInTurn)* (1 - this.facilityConstructionTimeRateDecrease)));
       ...
       public PersonList MovablePersons  //代替原来的Persons列表
       {
        ...
        public event Zhibao OnZhibao; //制作宝物
        public delegate void Zhibao(Treasure t, Person p);
        public void ZhiBao(Treasure t)
        {                   
            this.BelongedFaction.Leader.ReceiveTreasure(t);
            //Session.Current.Scenario.YearTable.addZhaoXianEntry(Session.Current.Scenario.Date, person, this.BelongedFaction.Leader);
            if (this.OnZhibao != null)
            {
                this.OnZhibao(t,this.BelongedFaction.Leader);
            }
            //t.BelongedPerson = this.BelongedFaction.Leader;
            //t.BelongedPersonIDString = this.BelongedFaction.LeaderID;           
        }
        //TreasureType type = new TreasureType();
        public void DoZhiBao(PersonGeneratorType preferredType)
        {
            bool isAI = !Session.Current.Scenario.IsPlayer(this.BelongedFaction);
            PersonGenerateParam param = new PersonGenerateParam(this, this.BelongedFaction.Leader, true, preferredType, isAI);
