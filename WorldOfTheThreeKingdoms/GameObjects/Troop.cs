...
namespace GameObjects
{
    ...
    public class Troop : GameObject
    {
        ...
        public InfluenceList GetAllInfluencesList()
        {
...
#pragma warning restore CS0067 // The event 'Troop.OnTroopLost' is never used
        ...
        public void AddMoveAnimationIndex(int steps)
        {
            if (this.Action == TroopAction.Move)
            {
                ...
                if (this.moveFrameIndex >= Session.GlobalVariables.TroopMoveFrameCount- (10 - Setting.Current.GlobalVariables.TroopMoveSpeed))
                {
        ...
        */
        ...
        public void DoCombatAction()
        {
            switch (this.Will)
            {
                case TroopWill.行军:
                    ...
                    //{
                    //    this.OrientationTroop=this.TargetTroop; this.CastTroop(this.OrientationTroop);
                    //}
                    else
                    {
                ...
                if (damage.OfficerInjury > 0 && damage.DestinationTroop.Persons.Count > 0 && !damage.DestinationTroop.ImmunityOfDieInBattle)
                {
                    ...
                    if (toInjure.InjureRate < 0.05 && Session.GlobalVariables.OfficerDieInBattleRate > 0)
                    {
                        ...
                        else
                        {
                            ...
                            if (!toInjure.ImmunityOfDieInBattle)
                            {
                                if ( toInjure.Tenacity<=0)
                                {
                                    toInjure.KilledInBattle(damage.SourceTroop);
                                }
                                else { toInjure.Tenacity--; }
                            }
                        }
                    }
                }
        ...
        {
            if ((Session.Current.Scenario.IsKnownToAnyPlayer(this) || Session.Current.Scenario.IsKnownToAnyPlayer(architecture)) && Session.MainGame.mainGameScreen.TileInScreen(architecture.Position))
            {
        ...
        {
            if ((Session.Current.Scenario.IsKnownToAnyPlayer(this) || Session.Current.Scenario.IsKnownToAnyPlayer(troop)) && Session.MainGame.mainGameScreen.TileInScreen(this.position))
            {
                if (this.CombatMethodApplied)
                {
        ...
        private void StartCastSelf()
        {
            ...
            if ((Session.Current.Scenario.IsKnownToAnyPlayer(this))&&Session.MainGame.mainGameScreen.TileInScreen(this.position))
            {
        ...
        public bool ToDoCombatAction()
        {
            ...
            {
                case TroopWill.行军:
                    //if (this.TargetArchitecture == null && this.TargetTroop == null && this.mingling==null) { break; }
                    if (this.CurrentStratagem == null)
                    {
                        flag = this.GetAttackOrientationObject(this.QueueEnded) != null;
                        break;
                    }
                    //if (this.CurrentStratagem.Self || this.CurrentStratagem.Friendly)
                    if (this.CurrentStratagem.Self)
                    {
                        flag = true;
                    }                    
                ...
                if (!this.Destroyed)
                {
                    ...
                    else
                    {
                        if(this.army!=null)
                        {
                            ...
                            // position updatd
                            runAnimation = runAnimation || (Session.Current.Scenario.IsKnownToAnyPlayer(this) && Session.MainGame.mainGameScreen.TileInScreen(this.position));
