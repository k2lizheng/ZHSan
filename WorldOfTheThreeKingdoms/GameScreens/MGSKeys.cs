    ...
    partial class MainGameScreen : Screen
    {
        ...
        private void HandleKey(GameTime gameTime)
        {
            ...
            if ((Session.Current.Scenario.CurrentPlayer != null) && Session.Current.Scenario.CurrentPlayer.Controlling)
            {
                ...
                else if (InputManager.KeyBoardState.IsKeyDown(Keys.V))
                {
                    this.currentKey = Keys.V;
                ...
                else if (InputManager.KeyBoardState.IsKeyDown(Keys.Z))
                {
                    ...
                    this.ShowTabListInFrame(UndoneWorkKind.Frame, FrameKind.Architecture, FrameFunction.Browse, false, true, false, false, Session.Current.Scenario.CurrentPlayer.Architectures.GetList(), null, "", "");
                }
                else if (InputManager.KeyBoardState.IsKeyDown(Keys.X))
                {
                    this.currentKey = Keys.X;
                    this.ShowTabListInFrame(UndoneWorkKind.Frame, FrameKind.Person, FrameFunction.Browse, false, true, false, false, Session.Current.Scenario.CurrentPlayer.Persons.GetList(), null, "", "");
                }
                else if (InputManager.KeyBoardState.IsKeyDown(Keys.Tab))
                {
                    if (this.CurrentArchitecture != null && this.CurrentArchitecture.BelongedFaction == Session.Current.Scenario.CurrentPlayer)
                    {
                        this.currentKey = Keys.Tab;
                        this.ShowFactionTechniques(this.CurrentArchitecture.BelongedFaction, this.CurrentArchitecture);
                    }
                }
                else if (InputManager.KeyBoardState.IsKeyDown(Keys.R))
                {
                    if (this.CurrentArchitecture != null && this.CurrentArchitecture.BelongedFaction == Session.Current.Scenario.CurrentPlayer)
                    {
                        this.currentKey = Keys.R;
                        this.ShowTabListInFrame(UndoneWorkKind.Frame, FrameKind.Architecture, FrameFunction.Browse, false, true, false, false, this.CurrentArchitecture.BelongedSection.Architectures.GetList(), null, "", "");                       
                    }
                ...
                else if (InputManager.KeyBoardState.IsKeyDown(Keys.F))
                {
                    if (this.CurrentArchitecture != null && this.CurrentArchitecture.BelongedFaction == Session.Current.Scenario.CurrentPlayer && this.CurrentArchitecture.BuildFacilityAvail())
                    {
                    ...
                    else if (this.LastArchitecture != null && this.LastArchitecture.BelongedFaction == Session.Current.Scenario.CurrentPlayer && this.LastArchitecture.BuildFacilityAvail())
                    {
                        this.currentKey = Keys.F;
                        this.CurrentArchitecture = this.LastArchitecture;
                        this.ShowTabListInFrame(UndoneWorkKind.Frame, FrameKind.Facility, FrameFunction.GetFacilityToBuild, false, true, true, false, this.CurrentArchitecture.GetBuildableFacilityKindList(), null, "选择设施-"
                            +this.CurrentArchitecture.Name+"-技术"+ this.CurrentArchitecture.Technology+"-资金"+ this.CurrentArchitecture.Fund, "");
                    }
