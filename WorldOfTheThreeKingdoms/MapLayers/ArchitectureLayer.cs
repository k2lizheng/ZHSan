    ...
    public class ArchitectureLayer
    {
        ...
        private PlatformTexture huoqujianzhutupian(Architecture architecture)
        {
            ...
            else
            {
                //try
                //{
                if ((architecture.JianzhuGuimo == 13 || architecture.JianzhuGuimo == 9) && Session.MainGame.mainGameScreen.Textures.largeCityImg.ContainsKey(architecture.Kind.ID))
                {
                    tupian = Session.MainGame.mainGameScreen.Textures.largeCityImg[architecture.Kind.ID];
                }
                else if ((architecture.JianzhuGuimo == 5 || architecture.JianzhuGuimo == 4)&& Session.MainGame.mainGameScreen.Textures.mediumCityImg.ContainsKey(architecture.Kind.ID))
                {
